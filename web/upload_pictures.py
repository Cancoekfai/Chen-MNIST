...
def train_model():
    ...
    model = LeNet_5()
    model.cuda()
    model.load_state_dict(torch.load('model/Chen_mnist_LeNet_5.h5')) #load model
    
    img = cv2.imread('static/images/00.png')
    w0, h0, _ = img.shape
    ...
    global ZZ
    ZZ=int(prediction.cpu().numpy())


    #file_name = "C:\\Users\\nik.000\\flask-tutorial\\flaskr\\static\\images"
...
def upload():
    if request.method == 'POST':
        ...
        upload_path = os.path.join(basepath, 'static/images',secure_filename('00.png'))  #注意：没有的文件夹一定要先创建，不然会提示没有该路径
        f.save(upload_path)
